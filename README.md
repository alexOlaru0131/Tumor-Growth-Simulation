...
Tumor-Growth-Simulation is a research-driven project designed to simulate tumor evolution based on a randomized genetic code, using principles of Reinforcement Learning (RL). Tumors are abnormal masses of cells that grow on organs or tissues, require blood vessels for continued growth (angiogenesis), and often exhibit complex, variable growth behaviors.

## Overview

The simulation is centered around the `TumorEnv` class, which defines the tumor environment, initialization process, and its dynamic behavior. Additionally, support for parallel simulations has been implemented via the `runParallelSimulation()` function in `Simulation.py`.

Each tumor instance is generated with a unique, randomized genetic code, which influences the following biological characteristics:
- Density – how closely cells are packed
- Growth rate – how quickly new cells are added
